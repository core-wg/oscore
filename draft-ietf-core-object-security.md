...
* In step 2, the server generates R2 = S2 \|\| HMAC(K_HMAC, S2) where S2 is a 4 byte random byte string, and the HMAC is truncated to 4 bytes. Neither R2, S2 nor the derived first and second security contexts need to be cached.
