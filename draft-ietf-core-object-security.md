...
* Code. The Outer CoAP Code of an OSCORE message is POST or FETCH for requests with corresponding response codes. The use of FETCH reveals that the message has an Observe option, but this is also revealed by an Outer Observe option or by traffic analysis. Changing the Outer Code may be a denial-of-service attack by causing errors in the proxy processing. 
