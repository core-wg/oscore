...
Everytime a client issues a registration request, even if the same Token is used (see Section 3.3.1 of {{RFC7641}}), a new Partial IV MUST be used, and so the payload and OSCORE option are changed. The server uses the Partial IV of the new request as the 'request_piv' of new notifications. 
...
C. If Inner Observe is equal to one, then and only then is this a cancellation and the attribute-value pair stored in step B for that Token and Security Context is deleted.
