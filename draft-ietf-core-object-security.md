...
This section gives examples of deriving new security contexts by adding randomness to the input parameters pre-established between client and server; in particular Master Secret, Master Salt and Sender/Recipient ID (see {{context-derivation}}). The random input is transported between client and server in the 'kid context' parameter as described below, which allows new security contexts to be established with certain message exchanges. Note that when the ID Context is known by both client and server, the 'kid context' may be omitted, and conversely, the 'kid context' parameter can he used in a request to remind the server of the ID context, should it have been lost.
