...
CoAP is designed to work with various kinds of proxies performing supporting operations in constrained environments including forwarding, translations, etc. Securing CoAP on transport layer protects the entire message between the endpoints in which case CoAP proxy operations are not possible. In order to enable proxy operations, security on transport layer needs to be terminated at the proxy in which case the CoAP message in its entirety is
unprotected in the proxy.
...
OSCORE protects CoAP messages as described in this section. The construction is intended to support CoAP Observe notifications, CoAP endpoints changing roles (between Client and Server) and CoAP group communications {{I-D.ietf-core-oscore-groupcomm}}. Unless otherwise stated, the description below applies both to CoAP request and CoAP response.

The part of the CoAP message which is protected is formatted as OSCORE Plaintext (see XX), which is the plaintext of a COSE_Encrypt0 object, encrypted with an AEAD. As specified by COSE, the Additional Data in the AEAD consists of the external_aad (see XX). Note that the external_aad is generated both at the encrypting and decrypting side and is not sent in the message. 

The key used to encrypt a CoAP message is called the Sender Key. This key is derived from the Master Secret, Sender ID and optionally Master Salt as defined in XX. The assumptions for these input parameters are specified in (XX). With these assumptions, the Sender Key is unique for all encrypting endpoints.

The Sender Key is used to encrypt the request (in the role of CoAP Client) and the response (in the role of CoAP Server), in particular to notifications where there may be more than one response to a given request.

The nonce used in the encryption is generated as in XX. The sender ID used in the nonce is that of the endpoint encrypting a request or sending a notification, and with the Partial IV being increased for each message that each nonce is different. In case of encrypting a response, the nonce generated by the sender ID of the endpoint sending the request and the Partial IV of the request is used, i.e. the nonce used to verify the request. By construction, the server uses different nonce-series (parametrised by the Sender ID) when generating requests/notification compared to when answering to requests, so the Sender Key and nonce will be unique for each message. 

Note that this construction is not limited to single recipients.

## Protected Message Fields

OSCORE depends on a pre-established strong Master Secret which can be used to derive Sender Keys and nonces. Assuming that is true, and the keys are used for no more data than indicated in XX, OSCORE should provide the following guarantees:

* Confidentiality.  An attacker should not be able to determine the plaintext contents of a given OSCORE message (section 5.3).
...
#### UDP Message Binding Fields

##### Type

##### Message ID

#### TCP Message Binding Fields
