...
The ID Context (see {{context-hint}}) MAY be used to distinguish between security contexts. The methods for assigning Sender ID can be used for assigning the ID Context. Additionally, the ID Context can be generated by the client (see  {{master-secret-multiple}}). ID Context can be arbitrarily long. 
