...
As an alternative to caching R2, the server could generate R2 in such a way that it can be sent (in response #1) and verified (at reception of request #2) as the value of R2 it had generated. Such a procedure MUST NOT lead to the server accepting replayed request #2 messages. One construction is that the server generates a secret random HMAC key K_HMAC per set of static security context parameters associated to a client, everytime it receives a message from that client for which it does not recognize a security context (e.g. because of reboot, as mentioned in {{context-state}}) and does not have an existing K_HMAC. Such a key might have an expiration time, after which a new value must be generated. At most, this key is used until a run of the protocol is completed (after verification of request #2), or until it expires; after either of those cases, the K_HMAC associated to that particular client is updated. Steps below refer to modifications to {{master-secret-multiple}}:

* In step 2, the server generates R2 = S2 \|\| HMAC(K_HMAC, S2) where S2 is a random byte string, and the HMAC may be truncated. Neither R2, S2 nor the derived first and second security contexts need to be cached.
...
Note that the length of the S2 and the truncation of the HMAC is application specific. The application needs to set the lengths such the probability of its value being repeated is negligable; typically, at least 8 bytes long. 
